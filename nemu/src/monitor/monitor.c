...
static void welcome() {
  Log("Trace: %s", MUXDEF(CONFIG_TRACE, ASNI_FMT("ON", ASNI_FG_GREEN),
...
int check_func_byElf(rtlreg_t dest) {
  for (int i = 0; i < n_sym_func; i++) {
    if (dest >= sym_func[i].start &&
        dest < sym_func[i].start + sym_func[i].length) {
      if (dest == sym_func[i].start) {
        printf("0x%x:%*s [%s(%d,%d)]\n",cpu.pc, 4+(++level)*2,"call",sym_func[i].name,cpu.gpr[10]._32,cpu.gpr[11]._32);
        return 0;
      } else {
        printf("0x%x:%*s [%s ]\n",cpu.pc, 3+(level--)*2,"ret",sym_func[i].name);
        return 0;
      }
    }
  }
...
static int load_elf(Elf_func *sym_func) {
  ...
  int n = get_sym_func(elf_file, sym_func);
