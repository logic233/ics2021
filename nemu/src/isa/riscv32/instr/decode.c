...
def_THelper(compute) {
  ...
  // def_INSTR_TAB("0000000 ????? ????? 111 ????? ????? ??", amd);

  return EXEC_ID_inv;
}
def_THelper(compute_M) {
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("0000001 ????? ????? ??? ????? 01100 11", R     , compute_M);
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 01100 11", R     , compute);
